	...
	<div class="row align-items-center 
		...
		@if (Carbon\Carbon::now() < $event->start_date)
			<div class="col-12 col-md-8 text-center ml-auto mr-auto">
				<h1 style="font-family: 'Courgette', cursive;">{{ $event->title }} Starts in</h1>
				<br>
				<div class="clock"></div>
		...
		@elseif (Carbon\Carbon::now() > $event->end_date)
			<div class="col-12 col-md-8 text-center ml-auto mr-auto">
				<h1 style="font-family: 'Courgette', cursive;">{{ $event->title }} has been ended</h1>
			</div>
		@else

		<div class="col-12 col-md-8 ml-auto mr-auto border">
			<h1 class="text-center" style="font-family: 'Courgette', cursive;">{{ $event->title }}</h1>
			...
			@endif
			<form action="{{ route('events.members20') }}" method="POST" class="mb-5">
				@csrf
				...
				<div class="form-group">
					...
					<select name="first" required class="form-control" id="">
						<option value="">Choose your first preference committee</option>
						@foreach ($committees as $committee)
							<option
							@if ($committee->id == old('first'))
								selected
							@endif
				...
				<div class="form-group">
					...
					<select name="second" required class="form-control" id="">
						<option value="">Choose your second preference committee</option>
							@foreach ($committees as $committee)
								<option
								@if ($committee->id == old('second'))
									selected
								@endif
								value="{{ $committee->id }}">{{ $committee->name }}</option>
							@endforeach
